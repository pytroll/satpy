 ...
 *
 * MPPP is free software: you can redistribute it and/or modify it under the
 * terms of the GNU General Public License as published by the Free Software
 * Foundation, either version 3 of the License, or (at your option) any later
 * version.
 *
 * MPPP is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE.  See the GNU General Public License for more
 * details.
 *
 * You should have received a copy of the GNU General Public License along with
 * MPPP.  If not, see <http://www.gnu.org/licenses/>.
 */


/*
 * This is the python module interface to the get_channel utility, which allows
 * to grab a Seviri channel from MSG data (using the MSG library).
 * 
 */
...
msg_get_channels(PyObject *dummy, PyObject *args)
{
  ...
  exception = PyErr_NewException("satin.RegionError", NULL, NULL);

  if(got_hr)
    {
      if (SetRegion(&hr_region,region_file,HRV) > WARNING ) 
	{
          PyErr_SetString(exception, "Could not initialize HR region.");
          return NULL;
	}     
  ...
  if(got_nonhr)
	...
	{
          PyErr_SetString(exception, "Could not initialize region.");
          return NULL;
	}
  ...
  exception = PyErr_NewException("satin.ReaderError", NULL, NULL);

  if(got_hr && (SevHRVInit(time_slot,hr_region,&hr_seviri)!=OK))
    {
      PyErr_SetString(exception, "Cannot initialize HR Seviri reader.");
      return NULL;
    }
 
  if(got_nonhr && (SevInit(time_slot,region,bands,&seviri)!=OK))
    {
      PyErr_SetString(exception, "Cannot initialize Seviri reader.");
      return NULL;
    }
  ...
  if(got_hr && (SevReadHRV(hr_region, &hr_seviri) > WARNING))
    {
      PyErr_SetString(exception, "Cannot read HR Seviri data.");
      goto error;
    }
    
  if(got_nonhr && (SevRead(region, &seviri, bands) > WARNING))
    {
      PyErr_SetString(exception, "Cannot read Seviri data.");
      goto error;
    }
...
initpynwclib(void)
{
    (void) Py_InitModule("pynwclib", MsgMethods);
    import_array();
    Py_Initialize();
}
