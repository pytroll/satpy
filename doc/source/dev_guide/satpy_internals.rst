...
as equality between the sorted key/value pairs of the instances. If a DataQuery has one or more values set to `'*'`, the
corresponding key/value pair will be omited from the comparison.