...
def get_fill_value(dataset):
    ...
    fill_vals = [(np.uint, np.iinfo(np.uint64).max), (np.integer, np.nan)]
    for t, fill in fill_vals:
        if np.issubdtype(dataset.dtype, t):
            return dataset.attrs.get('_FillValue', fill)
    else:
        return np.nan
