...
class HRPTFile(BaseFileHandler):
    ...
    def _get_channel_data(self, key):
        """Get channel data."""
        data = da.from_array(self._data["image_data"][:, :, _get_channel_index(key)], chunks=self.chunks)
    ...
    def _get_navigation_data(self, key):
        ...
        if key['name'] == 'latitude':
            data = da.from_array(lats, chunks=self.chunks)
        else:
            data = da.from_array(lons, chunks=self.chunks)
        return data
