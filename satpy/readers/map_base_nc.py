...
"""EUMETSAT CO2M Multi-Angle Polarimeter (MAP) readers base class."""
...
class MapNCBaseFileHandler(NetCDF4FileHandler):
    """Base reader class for MAP products in netCDF format.
    ...
    def __init__(self, filename, filename_info, filetype_info, orthorect=False):
        ...
        #self._integrated_solar_irradiance = self['data/calibration_data/Band_av
    ...
    def _standardize_dims(self, variable):
        ...
        # lat/lon dimensions
        if 'ground_pixel' in variable.dims:
            variable = variable.rename({'ground_pixel': 'x', 'scanline': 'y'})
    ...
    def get_dataset(self, dataset_id, dataset_info):
        ...
        if var_key != 'data/geolocation_data_dem/longitude' and var_key != 'data/geolocation_data_dem/latitude':
            print(var_key)
            view_key = dataset_info['view']
        logger.debug('Reading in file to get dataset with key %s.', var_key)

        #if var_key == 'data/geolocation_data_dem/longitude': #and self.longitude is not None:
    ...
    #        longitude,
    #        latitude,
    #        SCAN_ALT_TIE_POINTS, data/geolocation_data_dem/longitud
    #        TIE_POINTS_FACTOR
    #    )
    ...
    def start_time(self):
        ...
        #except ValueError:
        #    start_time = datetime.strptime(self['/attr/sensing_start_time_utc'], '%Y-%m-%d %H:%M:%S.%f')
        return '20250703111239'#start_time
    ...
    def end_time(self):
        ...
        #except ValueError:
        #    end_time = datetime.strptime(self['/attr/sensing_end_time_utc'], '%Y-%m-%d %H:%M:%S.%f')
        return '20250703111539'#end_time
