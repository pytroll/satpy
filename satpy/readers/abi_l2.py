...
class NC_ABI_L2(BaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info):
        ...
        self.sensor = 'abi'
        self.nlines = self.nc['y'].shape[0]
        self.ncols = self.nc['x'].shape[0]
        self.coords = {}
    ...
    def get_dataset(self, key, info):
        ...
        if 'Rad' in self.nc.variables.keys():
            self.varname = 'Rad'
        elif 'CMI' in self.nc.variables.keys():
            self.varname = 'CMI'
        else:
            self.varname = info['readvar']

        radiances = self[self.varname]

        if self.varname in ['Rad', 'CMI']:
