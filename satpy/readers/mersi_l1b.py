...
N_TOT_IR_CHANS_LL = 6
PLATFORMS_INSTRUMENTS = {"FY-3A": OSCAR.MERSI_1,
                         "FY-3B": OSCAR.MERSI_1,
                         "FY-3C": OSCAR.MERSI_1,
                         "FY-3D": OSCAR.MERSI_2,
                         "FY-3E": OSCAR.MERSI_LL,
                         "FY-3F": OSCAR.MERSI_3,
                         "FY-3G": OSCAR.MERSI_RM}
...
class MERSIL1B(HDF5FileHandler):
    ...
    def get_refl_mult(self):
        ...
        if self.sensor_name == OSCAR.MERSI_RM:
    ...
    def _get_coefficients(self, cal_key, cal_index):
        ...
        coeffs = self[cal_key][cal_index] if self.sensor_name != OSCAR.MERSI_LL else self[cal_key]
    ...
    def _get_dn_corrections(self, data, band_index, dataset_id, attrs):
        ...
        if slope is not None and dataset_id.get("calibration") != "counts":
            ...
            slope = 0.01 if self.sensor_name == OSCAR.MERSI_1 and dataset_id["name"] == "5" else slope
    ...
    def get_dataset(self, dataset_id, ds_info):
        ...
        if dataset_id.get("calibration") == "reflectance":
            ...
            calibration_index = None if self.sensor_name == OSCAR.MERSI_1 else ds_info["calibration_index"]
    ...
    def _mask_data(self, data, dataset_id, attrs):
        ...
        try:
            ...
            valid_range[1] = 25000 if self.sensor_name == OSCAR.MERSI_2 and dataset_id["name"] in ["24", "25"] and \
                                      valid_range[1] == 4095 else valid_range[1]
            # Similar bug also found in MERSI-1
            valid_range[1] = 25000 if self.sensor_name == OSCAR.MERSI_1 and dataset_id["name"] == "5" and \
                                      valid_range[1] == 4095 else valid_range[1]
    ...
    def _get_ref_dataset(self, data, ds_info):
        ...
        data = coeffs[0] + coeffs[1] * data + coeffs[2] * data ** 2 if self.sensor_name != OSCAR.MERSI_LL else \
    ...
    def _get_rad_dataset(self, data, ds_info, datset_id):
        ...
        elif self.sensor_name == OSCAR.MERSI_RM and datset_id["name"] in mersi_rm_vis:
    ...
    def _get_bt_dataset(self, data, calibration_index, wave_number):
        ...
        elif self.sensor_name == OSCAR.MERSI_LL:
        ...
        if corr_coeff_a != 0:
            data = (data - corr_coeff_b) / corr_coeff_a if self.sensor_name != OSCAR.MERSI_1 else \
