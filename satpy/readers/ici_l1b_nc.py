...
class IciL1bNCFileHandler(NetCDF4FileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, **kwargs):
        ...
        try:
            (
                self.observation_azimuth,
                self.observation_zenith,
            ...
            self.observation_azimuth, self.observation_zenith = None, None
        try:
            (
                self.solar_azimuth,
                self.solar_zenith,
    ...
    def _fetch_variable(self, var_key):
        ...
        if var_key == 'cached_longitude' and self.longitude is not None:
            ...
            var_key == 'cached_observation_zenith'
            and self.observation_zenith is not None
        ):
            variable = self.observation_zenith.copy()
        elif (
            var_key == 'cached_observation_azimuth'
            and self.observation_azimuth is not None
        ):
            variable = self.observation_azimuth.copy()
        elif (
            var_key == 'cached_solar_zenith'
            and self.solar_zenith is not None
        ):
            variable = self.solar_zenith.copy()
        elif (
            var_key == 'cached_solar_azimuth'
            and self.solar_azimuth is not None
        ):
    ...
    def _perform_geo_interpolation(
        ...
        for horn in horns.values:
            satint = GeoInterpolator(
                ...
                (n_scan.values, n_samples.values),
            )
