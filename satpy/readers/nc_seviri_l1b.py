...
class NCSEVIRIFileHandler(BaseFileHandler, SEVIRICalibrationHandler):
    ...
    @property
    def start_time(self):
        rc_start_time=(self.nc.attrs['true_repeat_cycle_start_day'] +
		        self.nc.attrs['true_repeat_cycle_start_mi_sec'] +
		        self.nc.attrs['true_repeat_cycle_start_ma_sec'] +
		        self.nc.attrs['true_repeat_cycle_start_na_sec'])
        return rc_start_time
        
    @property
    def end_time(self):
        rc_end_time=(self.nc.attrs['planned_repeat_cycle_end_day'] +
		            self.nc.attrs['planned_repeat_cycle_end_mi_sec'] +
		            self.nc.attrs['planned_repeat_cycle_end_ma_sec'] +
		            self.nc.attrs['planned_repeat_cycle_end_na_sec'])
        return rc_end_time
    ...
    def _read_file(self):
        ...
        polar_radius = (self.nc.attrs['north_polar_radius'] + self.nc.attrs['south_polar_radius']) * 0.5
    ...
    def get_dataset(self, dataset_id, dataset_info):
        ...
        dataset.values=xr.DataArray(self.nc[dataset_info['nc_key']].values, dims=['y', 'x']).where(self.nc[dataset_info['nc_key']].values != 0).astype(np.float32)
