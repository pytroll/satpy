...
from satpy.readers.vii_utils import tie_points_interpolation, C1, C2, MEAN_EARTH_RADIUS
...
class ViiL1bNCFileHandler(ViiNCBaseFileHandler):
    ...
    def __init__(self, filename, filename_info, filetype_info, **kwargs):
        ...
        solar_zenith_angle = self['data/measurement_data/solar_zenith'].values
        solar_zenith_angle_on_pixels = tie_points_interpolation(solar_zenith_angle)
    ...
    def _perform_calibration(self, variable, dataset_info):
        ...
        if calibration_name == 'brightness_temperature':
            ...
            # Perform the calibration
            calibrated_values = self._calibrate_bt(variable, cw, a, b)
            new_variable = xr.DataArray(
                data=da.from_array(calibrated_values, CHUNK_SIZE),
            ...
            # Perform the calibration
            calibrated_values = self._calibrate_refl(variable, self.angle_factor, isi)
            new_variable = xr.DataArray(
                data=da.from_array(calibrated_values, CHUNK_SIZE),
                attrs=variable.attrs,
                name=variable.name,
    ...
    def _perform_orthorectification(self, variable, orthorect_data_name):
        ...
        try:
            ...
            orthorect_values_degrees = da.degrees(orthorect_data / MEAN_EARTH_RADIUS)

            corrected_values = da.from_array(variable + orthorect_values_degrees, CHUNK_SIZE)
        except KeyError:
    ...
    def _calibrate_bt(self, radiance, cw, a, b):
    ...
    def _calibrate_refl(self, radiance, angle_factor, isi):
