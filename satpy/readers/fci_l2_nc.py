...
class FciL2NCFileHandler(BaseFileHandler):
    ...
    def get_dataset(self, dataset_id, dataset_info):
        ...
        try:
            fci_variable = self.nc[var_key]
        except KeyError:
            logger.warning("Could not find key %s in NetCDF file, no valid Dataset created", var_key)
            return None

        # Put the values in a dask array
        data_values_dask = da.from_array(fci_variable, CHUNK_SIZE)

        # TODO in some of the test files, invalid pixels contain the value defined as "fill_value" in the YAML file
        # TODO instead of being masked directly in the netCDF variable.
        # TODO therefore NaN is applied where such value is found or (0 if the array contains integer values)
        # TODO the next 4 lines have to be removed once the product files are correctly configured
        try:
            data_values_dask[data_values_dask == dataset_info['fill_value']] = np.NaN
        except ValueError:
            data_values_dask[data_values_dask == dataset_info['fill_value']] = dataset_info['mask_value']

        # If the variable has 3 dimensions, select the required layer
        if len(fci_variable.shape) == 3:
            layer = dataset_info.get('layer', 0)
            logger.debug('Selecting the layer %d.', layer)
            data_values_dask = data_values_dask[layer, :, :]

        # Create the xarray DataArray
        variable = xr.DataArray(data=data_values_dask,
                                name=fci_variable.name,
                                dims=('y', 'x'),
                                attrs=fci_variable.attrs)

        # Manages the attributes of the dataset
