...
def jma_true_color_reproduction(img):
    ...
    _jma_true_color_reproduction(img.data,
                                 sensor=img.data.attrs['sensor'],
                                 platform=img.data.attrs['platform_name'])


@exclude_alpha
@on_dask_array
def _jma_true_color_reproduction(img_data, sensor=None, platform=None):
    ...
    ccm_dict = {'AHI': {'himawari-8': np.array([[1.1629,  0.1539, -0.2175],
                ...
                'ABI': {'goes-16': np.array([[1.1425,  0.1819, -0.2250],
    ...
    if sensor is None:
        raise ValueError("Missing sensor name.")
    if platform is None:
        raise ValueError("Missing platform name.")

    # Get the satellite-specific conversion matrix
    try:
        ccm = ccm_dict[sensor.upper()][platform.lower()]
    except KeyError:
        raise KeyError(f"No conversion matrix found for sensor {sensor} and platform {platform}")
