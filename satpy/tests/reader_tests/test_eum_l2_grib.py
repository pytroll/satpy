...
class Test_EUML2GribFileHandler(unittest.TestCase):
    ...
    @mock.patch("satpy.readers.eum_l2_grib.xr")
    @mock.patch("satpy.readers.eum_l2_grib.da")
    def test_fci_data_reading(self, da_, xr_):
        ...
        with mock.patch("builtins.open", mock.mock_open()) as mock_file:
            with mock.patch("satpy.readers.eum_l2_grib.ec", self.ec_):
                self.ec_.codes_get_values.return_value = np.ones(5568*5568)
                self.ec_.codes_get.side_effect = lambda gid, key: FAKE_FCI_MESSAGE[key]
                self.reader = EUML2GribFileHandler(
                    filename="test.grib",
                    filename_info={
                        "spacecraft_id": "1",
