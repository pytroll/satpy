...
class Test_FciL2NCFileHandler(unittest.TestCase):
    ...
    @mock.patch('satpy.readers.fci_l2_nc.xr')
    @mock.patch('satpy.readers.fci_l2_nc.da')
    def test_dataset(self, da_, xr_):
        ...
        self.reader.get_dataset(None,
                                {'file_key': 'test_one_layer', 'fill_value': -999, 'mask_value': 0})
        # Checks that the dask.array has been called with the correct arguments
        name, args, kwargs = da_.mock_calls[0]
        self.assertTrue(np.all(args[0] == np.ones((100, 10))))
        ...
        self.assertEqual(kwargs['attrs'], {'test_attr': 'attr'})
        self.assertEqual(kwargs['name'], 'test_one_layer')
        ...
        self.reader.get_dataset(None,
                                {'file_key': 'test_two_layers', 'layer': 1, 'fill_value': -999, 'mask_value': 0})
        # Checks that the dask.array has been called with the correct arguments
        name, args, kwargs = da_.mock_calls[0]
        self.assertTrue(np.all(args[0].values[1, :, :] == 2 * np.ones((100, 10))))
        self.assertEqual(args[1], CHUNK_SIZE)
        # Checks that xarray.DataArray has been called with the correct arguments
        name, args, kwargs = xr_.mock_calls[0]
        self.assertEqual(kwargs['attrs'], {})
        self.assertEqual(kwargs['name'], 'test_two_layers')
        self.assertEqual(kwargs['dims'], ('y', 'x'))
        # The 'data' argument must be the return value of dask.array after slicing
        self.assertEqual(kwargs['data']._extract_mock_name(), 'da.from_array().__getitem__()')
        ...
        invalid_dataset = self.reader.get_dataset(None,
                                                  {'file_key': 'test_invalid', 'fill_value': -999, 'mask_value': 0})
...
def suite():
    ...
    mysuite.addTest(loader.loadTestsFromTestCase(Test_FciL2NCFileHandler))
