...
def run_crefl(refl,
    ...
    """
    sensor = instru.get_one_instrument_from_attrs(refl.attrs)
...
class _VIIRSMODISCREFLRunner(_CREFLRunner):
    def _run_crefl(self, mus, muv, phi, solar_zenith, sensor_zenith, height, coeffs):
        instrument = instru.get_one_instrument_from_attrs(self._refl.attrs)
...
def _runner_class_for_sensor(sensor_name: str) -> Type[_CREFLRunner]:
    try:
        return _SENSOR_TO_RUNNER[instru.normalize_instrument_name(sensor_name)]
    except KeyError:
