...
class MarcuseCompositor(GenericCompositor):
    """Make a Marcuse product.
    ...
    if (pal == 2) {col1=OWNL;col2=OWNM;col3=OWNH;}
        ...
        if (cont == 0) {
            if (ind <= lVal) return lValCol
            if ((ind > lVal) && (ind < hVal)) return mValCol
            if (ind >= hVal) return hValCol;
        } else {
    ...
    function applyEnh(bArr) {
        highlightBurnscar(burnscarHighlight, burnscarThresholdLow, burnscarThresholdHigh, burnscarDesaturateBackdrop,
                          burnscarDarkenBackdrop);
        return satEnh([stretch(bArr[0], stretchMin, stretchMax), stretch(bArr[1], stretchMin, stretchMax),
                       stretch(bArr[2], stretchMin, stretchMax)]);
    }
    ...
    var Fire1OVL = [stretch((2.1 * B04 + 0.5 * B12), 0.01, 0.99) + 1.1, stretch((2.2 * B03 + 0.5 * B08), 0.01, 0.99),
                    stretch(2.1 * B02, 0.01, 0.99)];
    var Fire2OVL = [stretch((2.1 * B04 + 0.5 * B12), 0.01, 0.99) + 1.1,
                    stretch((2.2 * B03 + 0.5 * B08), 0.01, 0.99) + 0.25, stretch(2.1 * B02, 0.01, 0.99)];
    var Fire3OVL = [stretch((2.1 * B04 + 0.5 * B12), 0.01, 0.99) + 1.1,
                    stretch((2.2 * B03 + 0.5 * B08), 0.01, 0.99) + 0.5, stretch(2.1 * B02, 0.01, 0.99)];

    // Band combinations (To get quicker processing you should comment out all those you are not using in the Settings
    //further down)
    ...
    var fire2 = Fire2OVL;
    var fire3 = Fire3OVL;
    var fire4 = [stretch((2.1 * B04 + 0.5 * B12), 0.01, 0.99) + 1.1, stretch((2.2 * B03 + 0.5 * B08), 0.01, 0.99) +
                 (a(B11, B12) - 1) / 4, stretch(2.1 * B02, 0.01, 0.99)];


    // Used band combinations and mixing
    var layer1 = NIRSWIRColors2;
    var layer2 = NaturalColors;
    var layer1Amount = 100;
    var layer2Amount = 0;
    ...
    def __call__(self, projectables, *args, **kwargs):
        """Generate the composite."""
        projectables = self.match_data_arrays(projectables)
        b02, b03, b04, b05, b08, b8a, b11, b12 = projectables
        ...
        bgg = bgg.where(fire < 0, bgg + fire / 4)
        bgr.attrs = b12.attrs
        bgg.attrs = b8a.attrs
        bgb.attrs = b05.attrs
        return super(MarcuseCompositor, self).__call__((bgr, bgg, bgb), *args, **kwargs)
