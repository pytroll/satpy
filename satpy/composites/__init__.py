...
class TrueColorFire(GenericCompositor):
    ...
    def __call__(self, projectables, *args, **kwargs):
        ...
        _ch1 = (projectables[0] / 100.) * 2.5
        _ch2 = (projectables[1] / 100.) * 2.5
        _ch3 = (projectables[2] / 100.) * 2.5

        _ch1b = (projectables[4] / 100.)
        _ch1b = np.max(np.maximum(0, _ch1b - 0.1))
        _ch2b = (projectables[3] / 100.)
        _ch2b = np.max(np.maximum(0, _ch2b - 0.1))
        ...
        res = super(TrueColorFire, self).__call__((ch1, ch2, ch3), *args, **kwargs)
        return res
